...
def заменить_текст_на_странице(page, старый_текст, новый_текст, is_date=False):
    ...
    if not области:
        logger.warning(f"Текст '{старый_текст}' не найден на странице")
        return
    logger.info(f"Найдено {len(области)} вхождений '{старый_текст}'")
    ...
    for область in области:
        ...
        logger.info(f"Заменено '{старый_текст}' на '{новый_текст}' в координатах {область}")
